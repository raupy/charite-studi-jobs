...
map_p_tags <- function(links, date_regex) {
    ...
    if (length(date_indices) == 0) {
      ...
      split_list <- split(
        date_indices,
        ceiling(seq_along(date_indices) / 2)
        )
      res <- map(map(split_list, ~ .x[1]:.x[2]),
                 ~ p_tags_text[.x]
                 )
    }
    res
...
extract_features <- function(mapped_p_tags, date_regex) {
  ...
  dates <- map(cleaned_text,
               ~ str_extract_all(.x, date_regex)
               ) %>%
    unlist() %>%
    dmy()
  ...
  tibble(institute = cleaned_text,
         ...
         deadline = dates[-odds]) %>%
    mutate(institute = ifelse(
      str_detect(institute, "[a-z]"),
...
find_list_jobs <- function(url, search_string) {
  ...
  html <- content(resp)
  links <- html %>%
    html_nodes(".list-jobs") %>%
    html_nodes("li a")
  html_text <- links %>% html_text()

  search_result <- str_detect(html_text, search_string)
  jobs_tbl <- tibble()
...
get_job_details <- function(url, search_string = "") {
  ...
  job_details <- tibble(
    starting_date = job_facts[1],
    limited_for = str_extract(
      job_facts[2],
      pattern = or(one_or_more(DGT) %R%
                     SPACE %R%
                     one_or_more(WRD),
                   DMY)),
    hours_per_month = str_extract(
      job_facts[3],
      pattern = one_or_more(DGT)
      ) %>%
      as.numeric(),
    contains_search_string = str_detect(
      job_description,
      pattern = or(START, one_or_more(SPACE)) %R%
        search_string %R%
